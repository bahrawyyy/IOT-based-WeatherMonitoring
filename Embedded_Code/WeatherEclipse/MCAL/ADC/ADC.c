...
/*uint16_t ADC_read(uint8_t channel){
	//ADMUX |= (1<<MUX0); //Select ADC1 on PINA_1
...
switch(channel){
	...
	while(GET_BIT(ADCSRA,ADIF) == 0);  // As far as this flag equals once,that mean the while is true, it will stay here
	// Once the flag equals 1, it will get out of the loop and continue the rest of the code
	SET_BIT(ADCSRA,ADIF);   // Clear interrupt flag agian
	return ADC;
}*/
